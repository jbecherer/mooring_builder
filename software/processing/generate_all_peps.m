function [] = generate_all_peps(path2ganges, experiment, platform, chipod_or_gust, inst_sn, do_redo, do_parallel )
%%     [] = generate_all_peps(path2ganges, [experiment], [platform], [chipod_or_gust], [inst_sn], [do_redo], [do_parallel])
%
%  This functions automatically calculates epsilon based on pitot data for instruments selected
%
%  INPUT
...
for i = 1:size(data,1)
   ...
   exist_praw = exist([ path2ganges basedir '/proc/pitot_eps300sec.mat']);

   if ~exist_praw | do_redo
      try
          low_or_high = 0; % low frequency estimate
          save_spec   = 0; % shall the spectrum be saved
         generate_pitot_eps([path2ganges basedir], do_parallel,  timelims, low_or_high, save_spec);
        % db comment 
